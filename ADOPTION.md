...
| 5.25 | **Adjacent severity-ramp stops sit ~1.2–1.7:1 apart** | Inherent to five stops on a green→red path, and not a literal 1.4.11 failure — every stop clears 3:1 against the track it is drawn on. The gate reports the adjacencies as `NOTE`, not `fail`. The fix is not more hue, it is `<Meter showStep>`, which prints the step's *word* on the axis deuteranopia collapses. Resolved in 0.3.0 as a **toggle, not a decision**: `showStep` is three-state, and omitting it defers to the theme — off under `classic`, on under `eva`. Neither generation loses |
| 5.26 | **`hazard #b020ff` is violet** | Neither the Eva nor the ISO caution convention, both of which are amber-on-black. It has a defined meaning here — a level-5 event, one rung above `critical` — so it stays. Revisited in 0.3.0: the amber/black convention governs hazard *striping*, which is a different object from a state colour, and violet-past-red is a real convention for one (AQI uses purple and maroon for its worst bands). The actual defect was luminance — `#b020ff` is 3.54:1 on `panel-3`, too dark to carry a word. `eva` lifts it to `#d38bff` at 7.02:1; classic keeps its value |
